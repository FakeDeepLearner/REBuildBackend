...
@RestController
public class SMSUnlockController {
    private final TwilioCredentials twilioCredentials;

    private final OTPService otpService;

    @Autowired
    public SMSUnlockController(TwilioCredentials twilioCredentials, OTPService otpService) {
        this.twilioCredentials = twilioCredentials;
        this.otpService = otpService;
    }


    @GetMapping("/api/get_otp/unlock/{phone_number}")
    ...
    public int getPhoneOtpForReactivation(@PathVariable String phone_number){
        ...
        Message.creator(
                ...
                String.format("""
                        Your one time to unlock your account is %d.
    ...
    @PostMapping("/api/verify_otp/unlock/phone_number")
